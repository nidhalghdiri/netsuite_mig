...
async function processSingleInventoryItem(accountId, token, item, recordType) {
  try {
    console.log("processSingleInventoryItem called with:", {
      item,
      recordType,
    });
    ...
    console.log(
      "expandReferences process processSingleInventoryItem",
      recordType
    );
...
async function fetchAndExpandInventoryDetail(
  ...
  try {
    ...
    console.log("inventoryDetail - recordType", recordType);
...
async function fetchAndExpandInventoryAssignment(
  ...
  try {
    ...
    console.log("assignmentList - recordType", recordType);
...
export async function fetchSublistItem(accountId, token, itemUrl, type) {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/netsuite/transaction/${type}/fetch-item`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
