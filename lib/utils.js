...
const REFERENCE_FIELD_NEW_ID = {
  ...
  entity: "custentity_mig_new_internal_id",
  salesRep: "custentity_mig_new_internal_id",
  aracct: "custrecord_mig_new_internal_id_account",
  doc: "custbody_mig_new_internal_id",
  vendor: "custentity_mig_new_internal_id",
  discountitem: "upcCode",
};
const RECORDS = {
...
export async function expandReferences(accountId, token, record, recType) {
  ...
  for (const field of REFERENCE_FIELDS) {
    ...
    try {
      ...
      if (field === "item") {
        if (record[field]?.links) {
          link = record[field].links.find((l) => l.rel === "self")?.href;
        }
        if (isDiscountItemUrlStrict(link)) {
          recordType = "discountitem";
          newIdField = REFERENCE_FIELD_NEW_ID["discountitem"];
        } else {
      ...
      if (link) {
        ...
        var url = `https://${accountId}.suitetalk.api.netsuite.com/services/rest/record/v1/${recordType}/${record[field].id}`;
      }
...
function isDiscountItemUrlStrict(url) {
  const pattern =
    /https:\/\/\d+\.suitetalk\.api\.netsuite\.com\/services\/rest\/record\/v1\/discountitem\/\d+/;
