...
export async function POST(request) {
  ...
  console.log("[PurchOrd] AccountId: ", accountId);
  console.log("[PurchOrd] token: ", token);
  ...
  try {
    ...
    if (response.ok) {
      ...
      return NextResponse.json({
        status: "completed",
        data: result,
        message: "Getting Purchase Successfully",
      });
    } else {
      const errorText = await response.text();
      throw new Error(
        `Failed to fetching record: ${response.status} - ${errorText}`
      );
    }
  } catch (error) {
