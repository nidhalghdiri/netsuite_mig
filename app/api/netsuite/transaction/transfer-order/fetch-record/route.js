...
export async function POST(request) {
  ...
  console.log("[transferOrder] AccountId: ", accountId);
  console.log("[transferOrder] token: ", token);
  console.log("[transferOrder] internalId: ", internalId);
...
async function processItems(accountId, token, items) {
  ...
  for (const batch of batches) {
    const batchResults = await Promise.all(
      batch.map((item) => processSingleItem(accountId, token, item))
    );
...
async function processSingleItem(accountId, token, item) {
  try {
